...
public class RateService {
	...
	public Rate getLatest(String fromcur, String tocur) {
		...
		return convertToRate(new LatestRate(symbolFromcur, symbolTocur, rate, rateDate));
	}
	 ...
	 * @param quote
	 */
	public void addLatestRate(Rate quote) {
	...
	private static Rate convertToRate(RateEntity re) {
		return new Rate(re.getFromcur().getCode(), re.getTocur().getCode(), re.getValue(), re.getDate());
	}
	...
	@Transactional
	public void addHistoricalRates(List<Rate> quotes) {

		List<HistoricalRate> ratesToAdd = new ArrayList<>();

		for (Rate quote : quotes) {
