...
public interface HistoricalRateRepository extends HistoricalRateRepositoryCustom, JpaRepository<HistoricalRate, Integer>, QuerydslPredicateExecutor<HistoricalRate> {
	...
	@Query("SELECT min(date) as date,avg(value) as value FROM #{#entityName} where fromcur.code=:fromcur and tocur.code=:tocur and (date between :start and :end) group by extract(YEAR FROM date), extract(doy FROM date),extract(HOUR FROM date) order by min(date)")
	...
	@Query("SELECT min(date) as date,avg(value) as value FROM #{#entityName} where fromcur.code=:fromcur and tocur.code=:tocur and (date between :start and :end) group by extract(YEAR FROM date), extract(doy FROM date) order by min(date)")
