...
public class JwtAuthenticationTokenFilter extends OncePerRequestFilter {
    ...
    protected void doFilterInternal(HttpServletRequest request, HttpServletResponse response, FilterChain chain) throws ServletException, IOException {
        ...
        if (requestHeader != null && requestHeader.startsWith("Bearer ")) {
            ...
            try {
                subject = jwtTokenUtil.getSubjectFromToken(authToken);
            } catch (IllegalArgumentException e) {
                log.error("an error occured during getting subject from token", e);
        ...
        if (subject != null && SecurityContextHolder.getContext().getAuthentication() == null) {
        	log.debug("checking authentication for user " + subject);
        	...
        	User userDetails = new User(subject, "", jwtTokenUtil.getGrantedAuthoritiesFromToken(authToken));
            ...
            if (jwtTokenUtil.validateToken(authToken, userDetails)) {
                ...
                log.debug("authenticated user " + subject + ", setting security context");
